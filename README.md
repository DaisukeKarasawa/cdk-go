...
> **💡 環境統一について**: このプロジェクトはDockerを使用して統一された開発環境を提供します。ローカルにGoをインストールする必要はありません。
...
```bash
# Lambda コードを変更した場合
make build-docker  # Docker環境でビルド
make deploy        # デプロイ
...
| コマンド            | 説明                           |
| ------------------- | ------------------------------ |
| `make setup-dev`    | 統一開発環境セットアップ       |
| `make build-docker` | Docker環境でLambda関数ビルド   |
| `make test-docker`  | Docker環境でテスト実行         |
| `make clean-dev`    | 開発環境クリーンアップ         |
| `make build-lambda` | Lambda関数をビルド（ローカル） |
...
- [リファクタリング手順書](./docs/guides/refactoring.md) - 本格的なプロダクション対応への改善手順
...
基本的なブログAPIから本格的なプロダクション対応アプリケーションへの段階的改善：

- [リファクタリング手順書](./docs/guides/refactoring.md) - 包括的な改善プロセス
  - [準備段階](./docs/guides/refactoring/01-preparation.md) - 現状分析・計画策定
  - [コード品質改善](./docs/guides/refactoring/02-code-quality.md) - 構造化・ベストプラクティス
  - [パフォーマンス最適化](./docs/guides/refactoring/03-performance.md) - キャッシュ・Cold Start対策
  - [テスト追加](./docs/guides/refactoring/04-testing.md) - ユニット・統合・E2Eテスト
  - [インフラ改善](./docs/guides/refactoring/05-infrastructure.md) - CDK構造化・モニタリング
  - [デプロイとバリデーション](./docs/guides/refactoring/06-deployment.md) - 自動化・ロールバック
